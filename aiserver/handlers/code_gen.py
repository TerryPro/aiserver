...
class GenerateHandler(APIHandler):
    ...
    def post(self):
        try:
            ...
            suggestion = self.generate_suggestion(language, source, context, intent, options, output, variables, history)
            
            llm_logger.info(f"生成的建议长度: {len(suggestion)} 字符")
            
            # Save interaction
            if notebook_id and cell_id and intent:
                try:
                    self.ai_session_manager.save_interaction(
                    ...
                    logger.info(f"Saved interaction for cell {cell_id}")
                except Exception as e:
    ...
    def generate_suggestion(self, language, source, context, intent, options, output=None, variables=None, history=None):
        """
        ...
        user_prompt = construct_user_prompt(language, source, context, intent, options, output, variables, history)
